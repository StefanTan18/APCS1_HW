...
public class StatPrinter {
	...
	for (int i = 1; i < _frequency.size() - 1; i++) {
	    if ((_frequency.get(i).compareTo( _frequency.get(i - 1)) > 0) &&
	...
	//The runtime is O(n^2) quadratic.
	public void printHistogram( int longestBar ) 
	{
	    // uses helper method to find the maximum frequency
	    int freqLongest = max(_frequency);
	    double scale = ((double) longestBar) / freqLongest;
	    String histogram = "";
	    for (int i = 0; i < _frequency.size(); i++) {
		histogram += i + " : ";
		for (int j = 0; j < (int) (_frequency.get(i) * scale); j++) {
		    histogram += "*";
		}
		histogram += "\n";
	    }
